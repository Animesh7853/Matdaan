...
const authMiddleware = async (req, res, next) => {
    try {
        ...
        const user = await Candidate.findOne({ where: { email: decoded.email } });
...
router.post('/signup', async (req, res) => {
    ...
    try {
        ...
        return res.status(201).json({ msg: "success", Candidate: newCandidate.id, token: req.cookies.token });
    } catch (err) {
...
router.post("/signin", async (req, res) => {
    ...
    try {
        ...
        const candidateAddress = await CandidateAddress.findOne({ where: { candidateId: user.id } }) || null;
...
router.post('/add-candidate', authMiddleware, async (req, res) => {
    ...
    try {
        let candidateAddress = await CandidateAddress.findOne({ where: { candidateId: candidateId } });
